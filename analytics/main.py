...
- Context route returns:
    {
      asOf, contractId,
      h4_untaken_levels: [ {type: swing_high|swing_low, price, formed_at, untaken:true}, ... ],
      m15_sessions: {
        asian:   {high, high_at, low, low_at, count} | null,
        london:  {high, high_at, low, low_at, count} | null,
        asian_window_ct:  {start, end},
        london_window_ct: {start, end}
      },
      m15_prevclose_swings: {
        since_prev_close_ct: {start, end},
        levels: [ {type, price, formed_at}, ... ]
      },
      m15_open_fvgs: [ {type: bullish|bearish, start, end, formed_at, filled, filled_at}, ... ]
    }
"""
...
def retrieve_bars(
    ...
    js = r.json() or {}
    if isinstance(js, dict) and js.get("bars"):
        return js["bars"]
    if isinstance(js, list):
        return js
    return []
...
def _swing_points(bars: List[Dict[str, Any]], left: int = 2, right: int = 2) -> List[Dict[str, Any]]:
    ...
    for i in range(left, n - right):
        try:
            hi = all(_gx(bars[i], "h", "high") >= _gx(bars[i - k], "h", "high") for k in range(1, left + 1)) and all(
                _gx(bars[i], "h", "high") >= _gx(bars[i + k], "h", "high") for k in range(1, right + 1)
            )
            lo = all(_gx(bars[i], "l", "low") <= _gx(bars[i - k], "l", "low") for k in range(1, left + 1)) and all(
                _gx(bars[i], "l", "low") <= _gx(bars[i + k], "l", "low") for k in range(1, right + 1)
            )
        except Exception:
            continue
        if hi:
            swings.append({"type": "swing_high", "idx": i, "price": _gx(bars[i], "h", "high"), "time": _gx(bars[i], "t", "time")})
        if lo:
...
def api_bars():
    ...
    try:
        ...
        tf = request.args.get("tf", request.args.get("interval", DEFAULT_INTERVAL))
        ...
        series = [
            {
                "time": _gx(b, "t", "time"),
                "open": _f(_gx(b, "o", "open")),
                "high": _f(_gx(b, "h", "high")),
                "low": _f(_gx(b, "l", "low")),
                "close": _f(_gx(b, "c", "close")),
                "volume": _f(_gx(b, "v", "volume")),
            }
            for b in bars
        ]
        return jsonify({"contractId": cid, "interval": tf, "series": series, "count": len(series)})
    except requests.HTTPError as e:
        log(f"/api/bars upstream: {e}", "ERROR")
    ...
    try:
        ...
        if not start or not end:
            if mode == "weekly":
                ...
                end_dt = now
            start = fmt_iso_z(start_dt)
            end = fmt_iso_z(end_dt)
        ...
        if route == "live":
            ...
            effective_live = _decide_route(req_end_dt)
        ...
        if not bars and effective_live:
            bars = retrieve_bars(
                cid, fmt_iso_z(eff_start), fmt_iso_z(eff_end), unit, unit_number, include_partial=include_partial, live=False
            )
...
def api_indicators():
    try:
        ...
        # If market closed and user didn't specify, default auto_window=true
...
def context_levels():
    try:
        ...
        contract = request.args.get("contract")
        live = request.args.get("live", "false").lower() == "true"
        as_of = request.args.get("asOf")  # ISO; optional, default now
        ...
        # --- 4h bars for untaken levels (last 5d) ---
        start_h4_utc = asof - timedelta(days=5, hours=1)
        bars_h4 = retrieve_bars(cid, fmt_iso_z(start_h4_utc), fmt_iso_z(asof), 3, 4, live=live)
        swings = _swing_points(bars_h4, left=2, right=2)
        h4_levels: List[Dict[str, Any]] = []
        for s in swings:
            taken, _ = _breached_after(bars_h4, float(s["price"]), int(s["idx"]), str(s["type"]))
            if not taken:
                h4_levels.append({
                    "type": s["type"],
                    "price": s["price"],
        ...
        # --- 15m bars for sessions + prev-day close swings ---
        ...
        bars_15 = retrieve_bars(cid, fmt_iso_z(m15_fetch_start), fmt_iso_z(asof), 2, 15, live=live)

        def _slice(bars: List[Dict[str, Any]], start_ct: datetime, end_ct: datetime) -> List[Dict[str, Any]]:
            out: List[Dict[str, Any]] = []
            su = start_ct.astimezone(timezone.utc)
            eu = end_ct.astimezone(timezone.utc)
            for b in bars:
                try:
                    t = parse_iso_z(str(_gx(b, "t", "time")))
                except Exception:
                    continue
                if su <= t <= eu:
                    out.append(b)
        ...
        def _hl(bars: List[Dict[str, Any]]):
            if not bars:
                return None
            highs = [(_gx(b, "h", "high"), _gx(b, "t", "time")) for b in bars if _gx(b, "h", "high") is not None]
            lows = [(_gx(b, "l", "low"), _gx(b, "t", "time")) for b in bars if _gx(b, "l", "low") is not None]
            if not highs or not lows:
                return None
            hi_val, hi_t = max(highs, key=lambda t: t[0])
            lo_val, lo_t = min(lows, key=lambda t: t[0])
            return {"high": hi_val, "high_at": hi_t, "low": lo_val, "low_at": lo_t, "count": len(bars)}
        ...
        m15_since_prevclose_levels = [
            {"type": s["type"], "price": s["price"], "formed_at": s["time"]} for s in m15_swings
        ]
